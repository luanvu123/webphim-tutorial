...
<div class="row container" id="wrapper">
         <div class="halim-panel-filter">
            <div class="panel-heading">
               <div class="row">
                  <div class="col-xs-6">
                     <div class="yoast_breadcrumb hidden-xs"><span><span><a href="{{route('movie',$movie->slug)}}">{{$movie->title}}</a> » <span><a href="{{route('country',[$movie->country->slug])}}">{{$movie->country->title}}</a> » <span class="breadcrumb_last" aria-current="page">
               ...
               <div class="clearfix wrap-content">
                  ...
                  <div class="button-watch">
                     <ul class="halim-social-plugin col-xs-4 hidden-xs">
                        <!-- <li class="fb-like" data-href="{{url('xem-phim/'.$movie->slug.'/tap-'.$episode->episode)}}" data-layout="button_count" data-action="like" data-size="small" data-show-faces="true" data-share="true"></li> -->
                        @php
                 ...
                 <!--  <div class="collapse" id="moretool">
                  ...
                  <div class="title-block">
                     ...
                     <div class="title-wrapper-xem full">
                        <h1 class="entry-title"><a href="{{route('movie',$movie->slug)}}" title="{{$movie->title}}" class="tl">{{$movie->title}}
                        @if($movie->thuocphim=='phimbo')
                        Tập {{$episode->episode}}
                       @else
                       <br><!-- Chất lượng: {{$episode->episode}} -->
                        @endif
                     </a></h1>
                     </div>
                  </div>
                        ...
                        <div role="tabpanel" class="tab-pane active server-1" id="server-0">
                           <div class="halim-server">
                              <ul class="halim-list-eps">
                                ...
                                <li style="display: flex;"class="halim-episode"><span class="halim-btn halim-btn-2 halim-info-1-1 box-shadow">{{$ser->title}}</span></li></a>
                                <!-- Tap phim -->
                                 <ul class="halim-list-eps">
                        ...
                        <div class="halim-item">
                           <a class="halim-thumb" href="{{route('movie',$hot->slug)}}" title="{{$hot->title}}">
                             ...
                             @endif
                                 ...
                                 @if($hot->phude==0)  
                                    <ion-icon name="language-outline" aria-hidden="true"></ion-icon>Phụ đề
                                 {{-- @if($hot->season!=0)
                                 -Season {{$hot->season}}
                             @endif --}}
                                 @elseif($hot->phude==1)
                                 <ion-icon name="mic-outline" aria-hidden="true"></ion-icon>Thuyết minh
                                 {{-- @if($hot->season!=0)
                                 -Season {{$hot->season}}
                             @endif --}}
                                 @else
