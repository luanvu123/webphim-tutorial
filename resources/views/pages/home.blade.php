                            ...
                            <a class="halim-thumb" href="{{ route('movie', $hot->slug) }}" title="{{ $hot->title }}">
                                ...
                                <div class="icon_overlay">
                                    <span class="status">
                                        ...
                                        @if ($hot->phude == 0)
                                            <ion-icon name="language-outline" aria-hidden="true"></ion-icon>Phụ đề-Tập
                                            {{ $hot->episode_count }}/{{ $hot->sotap }}
                                        @elseif($hot->phude == 1)
                                            <ion-icon name="mic-outline" aria-hidden="true"></ion-icon>Thuyết minh-Tập
                                            {{ $hot->episode_count }}/{{ $hot->sotap }}
                                        @else
                                            Trailer
                                        @endif
                                ...
                                <div class="halim-item">
                                    <a class="halim-thumb" href="{{ route('movie', $mov->slug) }}"
                                        ...
                                        <span class="status">
                                            ...
                                            <!--  <i class="fa fa-play" aria-hidden="true"></i> -->
                                            @if ($mov->phude == 0)
                                                <ion-icon name="language-outline"></ion-icon>Phụ đề-Tập
                                                {{ $mov->episode_count }}/{{ $mov->sotap }}
                                                {{-- @if ($mov->season != 0)-SS{{$mov->season}}
                             @endif --}}
                                            @elseif($mov->phude == 1)
                                                <ion-icon name="mic-outline"></ion-icon>Thuyết minh-Tập
                                                {{ $mov->episode_count }}/{{ $mov->sotap }}
                                                {{-- @if ($mov->season != 0)
                           -SS{{$mov->season}}
                       @endif --}}
